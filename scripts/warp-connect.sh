...
if [ "$(grep -c ':59910:' /etc/passwd)" -ne 1 ]; then
  useradd --uid 59910 --no-create-home --shell /bin/bash cfwarp
fi
...
if [ "$(grep -c 'generated by cloudflare-warp' /etc/resolv.conf)" -lt 1 ]; then
  echo " >> [warp-connect] cloudflare-warp did not modify /etc/resolv.conf"
  echo " >> [warp-connect] check if this container run configuration provides NET_ADMIN capabilities!"
  exit 1
else
  echo " >> [warp-connect] cloudflare-warp modified /etc/resolv.conf okay to proceed."
fi
...
if [[ ${_cfwarp_connected} != true ]]; then
  echo " >> [warp-connect] unable to obtain Warp connection!"
  exit 1
fi
